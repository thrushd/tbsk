...
// Begin: yoink
// below taken from https://github.com/qmk/qmk_firmware/blob/master/keyboards/oddball/oddball.c
...
void pointing_device_init(void){
    ...
    //optical_sensor_set_config((config_optical_sensor_t){ 200 });
}
...
void pointing_device_task(void){
    ...
    if(scroll_pressed) {
        ...
        mouse_report.x = clamped_x; // flip direction here
        mouse_report.y = -clamped_y; // flip direction here
    }
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case KC_CPI1:
            on_cpi_button(CPI1, record);
            return false;

        case KC_CPI2:
            on_cpi_button(CPI2, record);
            return false;

        case KC_CPI3:
            on_cpi_button(CPI3, record);
            return false;
